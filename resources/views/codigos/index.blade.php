...
<nav class="top-menu navbar navbar-expand-lg">
            ...
            <form id="formCodigo" action="{{ route('codigos.store') }}" method="POST">
              ...
              <div class="row mb-3">
                ...
                <div class="col-md-3">
                  ...
                  <select id="selectLibrerias" name="librerias[]" class="form-select" multiple>
                  </select>
  ...
  <script>
    ...
    fetch('{{ route("librerias.api") }}')
        .then(r => r.json())
        .then(list => {
            const cont = document.getElementById('selectLibrerias');
            list.forEach(l => {
                const opt = document.createElement('option');
                opt.value = l.id;
                opt.innerText = l.nombre + (l.lenguaje ? ' ('+l.lenguaje+')' : '');
